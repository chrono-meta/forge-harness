...
<!-- ============================================================
     ZENODO UPLOAD FIELDS — copy-paste each block into the form
     ============================================================ -->
...
### Description
```
AI collaboration harnesses have become primary determinants of AI output quality. We present forge-harness, an open harness engineering toolkit addressing four failure modes: steel-quench (adversarial validation), source-grounding-audit (phantom claim detection), harvest-loop (session-to-harness self-evolution), and sim-conductor (pre-deployment transfer validation). Applied to forge-harness itself, the methods resolved 10 S/A-grade structural defects, detected 3 phantom claims, and confirmed multi-user autonomous deployment. forge-harness is available at https://github.com/chrono-code/forge-harness.
```

### Keywords (each on a new line)
```
harness engineering
...
Frontier search during the evaluation identified harness-evolver [Christi, 2026] — an independent Claude Code plugin implementing a 7-stage harness optimization loop with 6 agents. Cross-audit confirmed that harness-evolver and forge-harness independently converged on the same outer-loop architecture: field observation → adversarial critique → synthesis → integration → verification. The Stanford IRIS Lab Meta-Harness [arXiv:2603.28052] implements the same outer-loop principle at automated benchmark scale. Three independent implementations arriving at the same loop structure through different paths validates the architecture as a structural property of the harness engineering problem rather than an artifact of any single implementation's assumptions.
