...
_html_escape() { # HTML 텍스트 노드용. 순서가 중요하다 — & 가 먼저다.
  # 🟥 **파라미터 치환을 쓰지 않는다 — 박을 상수가 없는 자리다.** bash 5.2 부터 치환문의 `&` 가
  #    «매치된 텍스트» 로 확장된다. 어느 형태를 골라도 한쪽이 깨진다(실측 2026-09-21, 같은 입력
  #    `<b>&"x"</b>`, 기대 `&lt;b&gt;&amp;"x"&lt;/b&gt;`):
  #
  #                         bash 3.2 (맥 기본)                bash 5.3 (CI)
  #      ${x//</&lt;}       &lt;b&gt;…  ✅                  <lt;b>gt;…  ❌  ← & 가 매치로 확장
  #      ${x//</\&lt;}      \&lt;b\&gt;…  ❌ (백슬래시 잔류)   &lt;b&gt;…  ✅
  #      sed 's/</\&lt;/g'  &lt;b&gt;…  ✅                  &lt;b&gt;…  ✅
  #
  #    🟥 이 결함은 **로컬에서 구조적으로 안 보였다** — 맥 기본 `/bin/bash` 가 3.2 라 초록이었고
  #    CI(5.x)에서만 빨갰다(`[[feedback_wiring_surfaces_hidden_failures]]` 의 «로컬 초록 ≠ CI 초록»).
  #    sed 치환문에서도 `&` 는 «전체 매치» 라 `\&` 가 필요하고, 그건 두 버전 공통이다.
