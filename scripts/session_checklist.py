...
# v10 (codex round 9): every exclusion is an ENVELOPE VALIDATOR — the opening marker at line start AND the
# closing/companion mark the real Claude Code envelope always carries. A user utterance that merely starts
# with the same literal (a look-alike) has no closing mark and stays a row. (prefix, required companion)
# (prefix, companion, where): where="end" — the real envelope ENDS with the companion (nothing after the closing
# tag; `<command-name>deploy</command-name> is sample text…` has prose after it and is a row) · where="in" — the
# companion appears anywhere · where="" — the prefix sentence alone is the envelope.
EXCLUDE_ENVELOPES = (
    ...
    ("Caveat: The messages below were generated by the user while running local commands", "", ""),  # v6: the synthetic caveat sentence itself
)
...
def is_excluded(text, entry):
    ...
    for p, companion, where in EXCLUDE_ENVELOPES:
        if not stripped.startswith(p):
            continue
        if where == "" or (where == "in" and companion in text) or (where == "end" and text.rstrip().endswith(companion)):
            return True
