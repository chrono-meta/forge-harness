...
| Artifact | Reference |
|---|---|
| Paper v1.0.2 — methodology | Zenodo DOI [`10.5281/zenodo.22843702`](https://zenodo.org/records/22843702) (all versions: `10.5281/zenodo.20397565`) — 2-layer design, 6-axis framework, 4-agent orchestration, compounding loop, with empirical evidence. **arXiv: v1.0 was rejected at moderation (2026-09-06)** — 11 of 17 reference entries did not match the works cited (see the erratum in the record); v1.0.1 corrected those entries and v1.0.2 withdrew four claims the body had already qualified — **but only in the sections its changelog names: the v1.0.2 deposit's abstract, conclusion and Table 1 were left unrevised and still carry the withdrawn wording** (Table 1 still reports the multi-model consensus as `100% (4 / 4 top recommendations, 3 frontier models)` and the deployments row as `Independent practitioner deployments confirmed`; the abstract still reads `three independent deployments`, the one word that release's own withdrawal notice says it withdrew). v1.0.3 propagates that withdrawal into all three, and v1.0.3 is not public. **v1.0.3 was resubmitted 2026-09-19 (`submit/8101851`) and is on hold at moderation** — it has not been announced, so nothing of this paper is publicly posted on arXiv. v1.0.4 (2026-09-20) corrects a self-citation in v1.0.3 that pointed at a superseded version DOI, and has not been submitted. Do not read the rejection as an assessment of the methodology, and do not read any version after v1.0 as re-reviewed: none has cleared moderation |
| cs.SE companion — governance-gate methodology | **preprint, publicly posted** · Zenodo DOI [`10.5281/zenodo.22674575`](https://zenodo.org/records/22674575) (v1.2.2; the record's description carries the per-version change notes — v1.2 added the replication section and withdrew two previously reported results; v1.2.1 propagates that withdrawal into the abstract and the conclusion, which v1.2 had left unrevised; v1.2.2 adds Sec. 6.7 and downgrades the title and abstract accordingly; all versions `10.5281/zenodo.20680080` · CC-BY-4.0) · [`arXiv:2609.04218`](https://arxiv.org/abs/2609.04218) (cs.SE; submitted 2026-07-01, announced 2026-09). **v2 announced 2026-09-09** and is on the public record: it carries the v1.2.2 content, adds §6.7 (an independent-session re-test of ground-truth recall under condition-blind scoring, which **does not replicate** §6.2's strict-recall contrast — the two conditions come out one strict hit apart in 24), downgrades the title's last clause from *Evidence* to *a Test of*, and turns the abstract's closing claim into a **hypothesis**. 🟢 **The two deposits agree as of 2026-09-15 — but the earlier ✅ on this line was false for six days, and how it went wrong is the part worth keeping.** The **PDFs** always agreed (`766,831 B`, md5 `6fd229ea77b1bc439d6b266adbcf4cba`, unchanged throughout). The **abstracts did not**: arXiv v2 ends *"so process design as the dominant factor **remains a hypothesis, not a result of this paper**"*, while the Zenodo description still carried *"the results are consistent with the thesis that review-process design … **is the dominant factor**"* and omitted the §6.7 non-replication sentence entirely — while the *same record's* change-notes paragraph described §6.7 correctly, so the record contradicted itself. Measured 2026-09-15 by diffing both live surfaces string-by-string (leading 400 chars not contained; similarity 0.792; the divergence isolated to the closing claim). **Repaired 2026-09-15T23:36:32Z** by replacing paragraph 1 of the Zenodo description with the arXiv v2 abstract verbatim; verified on the **published** record API afterwards, not the form: DOI `10.5281/zenodo.22674575` unchanged, version count still 5 (**no new DOI minted**), metadata keys 13 with creators/license/keywords(8)/related(4)/version all surviving, file checksum unchanged, description 4,222 chars in 5 paragraphs, and six content assertions passing.
🟥 **Why the earlier check missed it, stated so it is not repeated**: it verified that the description *"was re-read after publication and none changed"* — i.e. it asked **"did this field change?"** when the question that mattered was **"does this field match the new PDF?"**. A field can be perfectly unchanged and perfectly wrong. The control was pointed at the wrong proposition, which is why it returned a confident ✅. |
