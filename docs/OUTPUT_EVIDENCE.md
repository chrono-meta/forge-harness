...
| cs.SE companion — governance-gate methodology | **preprint, publicly posted** · Zenodo DOI [`10.5281/zenodo.22674575`](https://zenodo.org/records/22674575) (v1.2.2; the record's description carries the per-version change notes — v1.2 added the replication section and withdrew two previously reported results; v1.2.1 propagates that withdrawal into the abstract and the conclusion, which v1.2 had left unrevised; v1.2.2 adds Sec. 6.7 and downgrades the title and abstract accordingly; all versions `10.5281/zenodo.20680080` · CC-BY-4.0) · [`arXiv:2609.04218`](https://arxiv.org/abs/2609.04218) (cs.SE; submitted 2026-07-01, announced 2026-09). **v2 announced 2026-09-09** and is on the public record: it carries the v1.2.2 content, adds §6.7 (an independent-session re-test of ground-truth recall under condition-blind scoring, which **does not replicate** §6.2's strict-recall contrast — the two conditions come out one strict hit apart in 24), downgrades the title's last clause from *Evidence* to *a Test of*, and turns the abstract's closing claim into a **hypothesis**. 🟥 **RETRACTED 2026-09-15 — the two deposits do NOT agree, and the check that said they did asked the wrong question.** The **PDFs** agree (same file, md5 `6fd229ea…`). The **abstracts do not**: arXiv v2 ends *"so process design as the dominant factor **remains a hypothesis, not a result of this paper**"*, while the Zenodo record's description still reads *"the results are consistent with the thesis that review-process design … **is the dominant factor**"* — and the §6.7 non-replication sentence (*"does not replicate that contrast: the conditions differ by one strict hit in 24"*) is **absent from the Zenodo abstract entirely**. The same record's *change-notes* paragraph does describe §6.7 correctly, so the record contradicts itself. Measured 2026-09-15 by diffing both live surfaces string-by-string (`arxiv.org/abs/2609.04218` abstract vs `zenodo.org/api/records/22674575` description; leading 400 chars not contained, similarity 0.792, the divergence isolated to the closing claim).
🟥 **Why the earlier check missed it, stated so it is not repeated**: it verified that the description *"was re-read after publication and none changed"* — i.e. it asked **"did this field change?"** when the question that mattered was **"does this field match the new PDF?"**. A field can be perfectly unchanged and perfectly wrong. The control was pointed at the wrong proposition, which is why it returned a confident ✅. Remedy is a Zenodo metadata edit (no new DOI — precedent 2026-09-09), pending operator execution; until then **the machine-readable `isIdenticalTo arXiv:2609.04218` relation over-claims** for the abstract field. The earlier divergence (arXiv v1.2.2 against Zenodo v1.2.1) is therefore closed; the superseded v1.2.1 remains separately citable at `10.5281/zenodo.22635721`. 🟥 arXiv is a moderated preprint server; moderation is not peer review, and neither the Zenodo deposit nor the arXiv posting establishes peer review, editorial acceptance, or venue acceptance |
