...
> **Multi-Team Adversarial Panel (measured 2026-05-31)**: Each available AI CLI forms an independent team; every team deploys 2–3 personas against the same artifact in parallel. Cross-team synthesis surfaces Claude blind spots — issues external teams catch that single-model review misses. **Measured on `source-grounding-audit/SKILL.md`**: C1 single-persona found 25% of defects; C2 cross-session 3-persona found 75%; C3 + Gemini team found 100% — including 3 Claude-only blind spots (one S-grade). Claude-side token cost did not increase C2→C3; Gemini billed to its own quota. Decision rule: routine→C2 (cross-session), pre-publish→C3+ (zero Claude overhead, +25% coverage). Falls back gracefully: external CLI → cross-session claude → skip. Note: CLI presence check is insufficient — headless operability must be verified (Codex was present but TTY-required). Corporate path: `gh copilot` as sidecar (GitHub Copilot CLI, separate enterprise license).
