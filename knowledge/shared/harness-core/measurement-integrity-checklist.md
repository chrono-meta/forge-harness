...
| n+13 | **A live instrument judges by COUNT — the count is right, the referent is wrong** | A pty probe counted OSC 777 notifications to decide «the hook's alert reached the terminal». The CLI emits its own OSC 777 (`…needs your approval for the plan`), so the known-negative (hook without the alert field) scored 1 — a true count of the wrong thing (2026-09-26, field harness, CLI 2.1.283) | The known-negative scores non-zero. Judge by **prefix / identity** (the body starts with the hook's own title), never by how many arrived — `hook_channel_visibility.md` §3 |
| n+14 | **The defect axis is two-sided, and the instrument minimizes along it** | A loop-seam ratio (last→first frame gap ÷ neighbour step) was minimized to pick the cut point; 0.00–0.02 read as «perfect» but meant a duplicated frame — a freeze. `≫1` = skip, `≈1` = normal, `≈0` = freeze, so minimizing *produces* the other defect (2026-09-23, field harness) | One known-negative is not enough: a two-sided axis needs **a negative on each side**. Target the normal value in log distance, or judge «is this value precedented inside this target's own distribution» instead of a threshold |

**After fixing how an instrument READS, grep the repo for the symptom string** (the API call or
token the broken reading used, e.g. `getComputedStyle(..).fontSize`), and dispose of every hit as
ⓐ same defect → fix in the same commit · ⓑ not affected → one line saying why · ⓒ undecided → name it
in a signal. Grep the *string*, not «the same class» — the author is the one who just failed to
generalize (2026-09-24, field harness: the same dead reading, fixed in one probe, was still live in the
neighbouring probe and broke two legs in opposite directions).
