...
| "지난주에 뭐 했지", "what did we do last week", "예전에 이거 한 적 있나", **"find past work on X", "search for prior X", "이전에 한 작업 찾아줘"** (recall intent — 일반형, 2026-09-22 확장) | **CATALOG-first recall** — read `CATALOG.md`, identify candidates by tag/date, then open only those files. Never scan session files one by one |
