...
<p align="center">
  ...
  一个实践者的 <b>元框架 (meta-harness)</b> —— 你的项目框架们所栖居的太阳系。<br>它抬高每个项目的 <b>下限 (floor)</b>（把设置框架化）和 <b>上限 (ceiling)</b>（加速工作），再把这些收益在你的整个项目组合中复利累积。
</p>
...
规模才是重点。**技能、agent 或插件** 是一个工具。**框架** 高出一级 —— 是一颗 *星*：
一个项目的工具、规则、门禁与记忆，绑成一个运作的整体。**forge-harness 就是这些星所栖居的
太阳系** —— 它把众多框架收进同一个引力井，让它们保持在轨（共享的下限、无漂移），并让它们
一起演化而不是四散。这就是它所朝向的目标。实际上，这份引力来自四件事：
