...
- **Skill skeleton** — the frontmatter field set (`name`/`description`/`user-invocable`/`allowed-tools`/`model`) that is universal across active FH skills (regen: `grep -L user-invocable plugins/*/skills/*/SKILL.md` returns none) + Trigger stub (≥3 phrases) + Done-When stub (with check-class) + Step skeleton. **Parameterizes** `contention-layer` Step 4's skeleton template (per its need-driven note — `origin: field-scaffold`, `contention-parents`→source-pattern pointer).
