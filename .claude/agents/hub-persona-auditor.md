...
> **Dual registration**: Lightweight version available directly in the FH cwd — external installs use `plugins/fh-meta/agents/hub-persona-auditor.md` version (no plugin install required).
...
| Hub mission layers (frontier translation + AI collaboration guide) | User's own mission/value baseline |
| Organization-specific audience classification | User's own audience classification |
