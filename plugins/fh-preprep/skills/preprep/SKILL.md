...
| **L15-b font-embed** | **쓰는 서체를 덱이 «들고 다니나»** — `presentation.xml` 의 `embeddedFont` 집합이 `slides/` 가 부르는 서체 집합(`a:latin`·`a:ea`, 테마 참조는 풀어서)을 덮나. 🟥 L15 와 **다른 실패**를 잡는다: L15 는 «템플릿 집합을 벗어났나», 여기는 «발표장 PC에 설치돼 있다고 가정하고 있나». 실사고 2026-09-21: 한 덱이 **안 쓰는 서체를 임베드하고 실제로 쓰는 둘(런 3,724회)을 안 넣은 채** 제출됐다 | 차단 (🟥 **임베드 0 = UNMEASURED 아니라 FAIL** — 이 축은 부재가 곧 결함이다. 면제는 `fonts.embed_exempt`(+`why`) · 임베드했지만 미사용은 노트로만 · `a:sym`/`a:cs` 전용 지정과 못 푼 테마 참조는 **UNMEASURED**) |
...
🟥 **정정 2026-09-21 — 둘째의 «절반»은 파일 «안»의 사실이라 이제 잰다.** 설치 여부는 여전히 못 보지만
**«설치에 기대고 있나»(=임베드했나)는 잰다** — 그 자리가 **L15-b font-embed** 다. 즉 위 문장은
«못 본다»가 아니라 **«못 보는 쪽과 보는 쪽이 갈린다»**로 읽어야 한다. 이 구분 없이 인용하면
임베드 결함을 «구조적 한계»로 덮는다(실측: 한 덱이 실사용 서체 둘을 안 넣고 제출됐다).
