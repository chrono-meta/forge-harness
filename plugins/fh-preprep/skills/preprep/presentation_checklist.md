...
- [ ] 🟢 **A7. 제목은 «주제»의 표지다 — «소재»의 표지가 아니다.** (2026-09-21 신설)

      🟥 **축이 둘이고, 뭉개면 제목을 읽을 수 없다.**
      · **소재(素材)** — 지금 다루고 있는 «대상». 연속 페이지가 공유한다.
      · **주제(主題)** — 그 소재에 대해 «이 장이 말하는 것». 장마다 같을 수도 다를 수도 있다.

      **제목은 «주제»와 함께 움직인다.**

          소재 같음 + 주제 같음  →  🟢 제목 고정.
                변화는 **본문**이 지고, 그 변화의 «의미»는 **대본**이 나른다.
                같은 템플릿이라는 «지붕» 아래서 논지가 이어진다.
          소재 같음 + 주제 다름  →  🟢 제목 변경이 «허용»된다. **표지가 일을 하는 자리**다.
          🟥 소재도 주제도 같은데 제목만 바꾸면  →  표지가 **거짓 경계**를 만든다.
          🟥 주제가 바뀌었는데 제목이 그대로면  →  청중이 **넘어간 걸 못 읽는다.**

      🟥 그러므로 **「제목을 같게 하라」도 「다르게 하라」도 규칙이 아니다.** 규칙은
      **「제목이 주제와 함께 움직이게 하라」**다. 제목을 바꾸는 것도, 안 바꾸는 것도
      **공짜가 아니라 신호를 쓰는 것**이다.

      실례 셋 — 전부 한 덱 실물에서 나왔다:

          연속 5장(그리고 이어지는 4장)   소재 같음 · 주제 같음  → 제목 동일. **지붕**
          17장짜리 한 묶음                소재 같음 · 주제 다름  → 제목 변경 허용. 표지가 일한다
          방법론 a·b·c                    제목은 다르되 **상위 축(같은 방법론)은 공유**
                                          ⇒ «제목 변화 = 단절»이 아니다.
                                             **표지는 경계를 가리키지 단절을 선언하지 않는다**

      🟢 **이건 작성 규칙이자 «읽는 렌즈»다.** 저자 말대로 *「이 관점에서 보면 제목 구분이
      확연히 보인다」* — 이 축으로 한 번 훑으면 덱의 지붕 구조가 드러나고, 그것이
      «안정적이고 일관성 있는 완성도 높은 장표»를 만든다.

      🟥 **그리고 이 자리가 감사자를 틀리게 만든다.** 스캔한 쪽이 위 «연속 5장»을
      «복사-붙여넣기 실수 · 논지가 안 선다»로 올렸다. 저자 판정: *「의도한 수정이고,
      깔끔하게 보이기 위해서 — 대본이 설명해주니까」*.
      🟥 **오독의 정확한 이름**: **«주제가 같다»를 확인하지 않고 «제목이 같다»만 봤다.**
      두 축을 안 갈랐기 때문에 표지를 읽을 수 없었다. ⇒ §E10 의 «판정에 필요한 맥락이
      슬라이드 밖에 있다(대본)»의 **실물 근거**가 이것이다.

      ⚠️ **레인으로 만들지 마라** — 「이 두 장의 **주제**가 같은가」는 파일 안에서 기계로
      못 가른다(제목이 같은지는 읽히지만 그건 다른 축이다). 억지로 만들면 그게 장식이다.
      가능한 것은 **관측 한 줄**뿐이다: «연속 N장 제목 동일» 분포를 **세어서 보여주기만**
      (판정 없이). 저자가 자기 덱의 지붕 구조를 눈으로 보는 용도다.
...
- [ ] **B4. 진행 상태는 «명도»로. 강조색에 겸업시키지 마라.** (2026-09-21 개정)
      초판은 *「지나간 것 흰색 · 지금 노랑 · 아직 회색」* 이었다. 🟥 **「지금 노랑」을 뺀다** —
      브랜드 강조색이 «진행 표시»와 «내용 강조» 두 일을 하면 어느 쪽도 안 읽힌다(§E8 A1).
      실측(2026-09-21): 외부 디자인 검수본이 간지 **6/6 전수**에서 현재 항목을
      강조색 → 밝은 회색으로 바꾸고 나머지를 한 단계 낮췄다. 예외 0.
      ⚠️ **「지나간 것 / 아직인 것을 가르라」는 열린 채로 둔다** — 개정 전 판도 개정 후 판도
      그 둘을 **같은 값**으로 두는데 운영자는 결과가 맞다고 판정했다. 그러면 의심 대상은
      덱이 아니라 **이 항목 자체**다. 🟥 **명도 3단계는 제안이지 관측이 아니다** — 아무도
      안 썼다. 관측 아닌 것을 전수 근거로 인용하지 마라.
      🟥 **그리고 대비 수치로 이 항목을 판정하지 마라** — §E10 을 먼저 읽어라.
...
- [ ] 🟥 **E10. 검출은 기계가, 판정은 사람이 — 그리고 대비 비율은 «판정»이 아니라 «후보 선별»이다.**
      (2026-09-21 신설. 실측에서 역산했고, 그 실측은 **감사자가 4/4 틀린 기록**이다)

      **① 덱 대조는 «검출»까지가 계기의 몫이다.**
      · 델타 검출·분류는 기계가 잘한다 — 그 실측에서 **120장 중 바뀐 5장을 정확히 집었다(놓침 0)**.
      · «이게 실수인가 의도인가»는 기계가 못 한다 — 같은 실측에서 **«저쪽 실수»로 올린 넷이
        4/4 의도**였다.
      · 🟥 공통 원인은 하나다 — **슬라이드만 보고 판단했다.** 판정에 필요한 맥락이 슬라이드 밖에 있다:
          **대본**(제목이 같은 연속 장 → §A7) · **실물 크기 화면**(딤 프레임·간지) · **요건 정의**(고지 문구)
      ⇒ **보고는 «무엇이 달라졌나»로 쓰고 «무엇이 잘못됐나»로 쓰지 마라.**
        후자를 쓰려면 **그 세 맥락을 먼저 열었는지 스스로 물어라.**

      **② 대비 비율은 문턱을 넘었나로 판정하지 마라.**
      · 문턱 미달 = 「볼 값어치가 있다」이지 **「결함이다」가 아니다.**
      · 문턱값(WCAG 4.5:1 · 3:1)은 **본문 크기 전제**다 — 간지·표지의 **큰 글자**,
        **의도된 딤 프레임**에는 그대로 안 걸린다.
      · 종단 판정은 **«실물을 실제 크기로 본 사람»**이다.
      🟥 실례: 간지 비현재 항목 **2.03:1**, 딤 프레임 부제 **1.46:1** 을 «안 읽힌다»로 올렸으나,
      저자가 실물 화면에서 **«다 읽히고 전부 의도대로»**로 판정했다. **수치는 맞았고 결론이 틀렸다.**
      계기는 틀리지 않았다 — **계기가 «그 질문»의 계기가 아니었다.**

      **③ 렌더 해상도가 그 오판을 키운다.** 위 판정은 **55dpi** 래스터에서 나왔다. 그 해상도에서
      한글 획은 배경과 섞이므로 «이게 읽히나»를 그 래스터로 판정하면 **실제보다 나쁘게 나온다.**
      ⇒ 읽힘 판정용 렌더는 **실제 투사 크기에 가까운 해상도**로 굽거나, 그 판정을 **사람에게 넘겨라**.
      (§E9 가 «페르소나에게 렌더를 줘라»라면, E10 은 «그 렌더로 무엇을 판정해도 되나»다.)

      🟢 **그리고 반증이 가능했던 이유를 같이 적는다** — 보고에 *「이 수치가 «안 보인다»는 뜻인지는
      장비가 정한다. 나는 숫자까지만 낼 수 있다」* 가 붙어 있었고, 그래서 저자가 실물로 뒤집을 수
      있었다. 그 줄이 없었으면 **네 번 잘못 고쳤다.**
      ⇒ **계량 보고에는 «이 수치가 답하지 못하는 질문»을 같이 적는다. 그게 반증 경로다.**
...
- [ ] **L1. 제목이 두 줄로 접히는 원인은 글자 크기가 아니라 「상자 폭」인 경우가 많다.**
      `ext cx` 가 8M 인 제목 상자가 섞여 있어 6200 이 접혔다.
      🟥 **정정(2026-09-21) — 처방이 「폭 통일」이 아니다.** 초판은 *「폭을 먼저 통일(22M)하고
      그 다음에 길이별 크기를 정하라」* 였는데, 외부 디자인 검수본은 **고정폭을 버렸다** —
      제목 111장 전수가 `wrap="none"` + `spAutoFit` 이라 **상자 폭이 장마다 다르다**(글자를 감싼다).
      ⇒ 처방은 **«폭을 고정하지 말고 글자를 감싸게 하라»**(§L2)이고, 고정폭 통일은 그 다음
      선택지다. **고정폭이 접힘의 원인이었다**는 진단만 유효하다.
- [ ] 🟥 **L2. 오토핏은 둘이고 서로 반대다 — 한 낱말로 묶지 마라.** (2026-09-21 정정)
      · `normAutofit` = **글자를 상자에 맞춘다**(줄여 버린다) → 🟥 **꺼라.** 켜져 있으면 접힌 뒤
        글자가 줄어서 **같은 크기 지정이 화면에서 다른 크기로 보인다.**
      · `spAutoFit` = **상자를 글자에 맞춘다**(상자가 자란다) → 🟢 **켜는 게 맞다.**
        `wrap="none"` 과 짝지으면 제목 상자가 글자를 감싸고, 길이가 달라도 글자 크기는 안 변한다.
      ⚠️ 초판은 이 둘을 *「`wrap="none"` + 오토핏 해제」* 한 줄로 묶어서, **그대로 집행하면
      맞는 설정(`spAutoFit`)까지 되돌리는** 지시가 돼 있었다. 실측(2026-09-21): 외부 디자인
      검수본이 제목 111장 전수를 `wrap="none"` + **`spAutoFit` 켬**으로 바꿨다 — 즉 전문가는
      «상자를 글자에 맞추는» 쪽을 골랐다.
- [ ] **L3. 세로 앵커도 제목의 일부다.** 한 장만 `bodyPr anchor="ctr"` 이면 그 장의 제목이
      **아래로 내려앉는다**(상자 높이의 절반만큼). 실측 8장이 그랬고, 운영자가 «14페이지가
      13페이지보다 내려왔다»로 잡았다.
      🟥 **정정(2026-09-21) — 원인은 `ctr` 이 아니라 «혼용»이다.** 초판은 *「전수로 `anchor="t"` 로
      맞춰라」* 였는데, 외부 디자인 검수본은 **전수 `anchor="ctr"`** 로 갔고 어긋남이 없다.
      ⇒ 규칙은 **«하나로 통일하라»**이지 «`t` 로 하라»가 아니다.
      🔷 그리고 `spAutoFit`(§L2)과 짝이면 **`ctr` 쪽이 강하다** — 상자 높이가 장마다 달라지므로
      `t` 로 맞추면 위가 붙고 아래가 들쭉날쭉해진다. 실측: 그 판은 `y` 가 두 값이었지만
      **`y + cy/2`(세로 중심)가 111/111 전수 동일**이었다.
      ⇒ 🟥 **자리 고정은 `y` 가 아니라 «세로 중심»으로 판정하라.** 오토핏 상자를 `y` 원값으로
      대조하면 정상인데도 «어긋남»이 뜬다(그 판에서 89 : 22 로 갈렸다).
