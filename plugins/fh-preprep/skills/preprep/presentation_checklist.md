...
- [ ] 🟢 **A7. 제목은 «맥락 경계의 표지»다 — 매 장 바꾸면 표지가 무의미해진다.** (2026-09-21 신설)
      · 같은 맥락이 이어지는 **연속 페이지는 제목을 고정**한다. 변화는 **본문**이 지고,
        그 변화의 «의미»는 **대본**이 나른다.
      · 제목 교체는 **«맥락이 실제로 바뀌는 자리»**에서만 한다.
      🟥 매 장 제목을 바꾸면 그 표지가 무의미해진다 — 청중이 «어디서 넘어갔는지»를 읽을
      단서를 잃는다. **제목을 다르게 쓰는 것은 공짜가 아니라 신호를 쓰는 것이다.**
      · 반대 방향도 규칙 안에 있다: **제목이 달라도 상위 축은 공유될 수 있다**(예: 같은
        방법론 안의 a·b·c). ⇒ «제목 변화 = 맥락 단절»이 아니다. **표지는 경계를 가리키지
        단절을 선언하지 않는다.**
      효과: 같은 템플릿이라는 **«지붕»** 아래서 논지가 이어진다.

      실례(2026-09-21 실물): 한 덱에서 **연속 다섯 장이 같은 제목**이었고, 그 다음 묶음
      **네 장이 또 같은 제목**이었다 — 각 장의 변화는 본문과 대본이 설명한다.
      🟥 **그리고 이 자리가 감사자를 틀리게 만든다** — 스캔한 쪽이 그 다섯 장을
      «복사-붙여넣기 실수 · 논지가 안 선다»로 올렸다. **대본을 안 보고 슬라이드만 봐서
      «지붕»을 «실수»로 읽은 것이다.** 저자 판정: *「의도한 수정이고, 깔끔하게 보이기 위해서 —
      대본이 설명해주니까」*. ⇒ §E10 의 «판정에 필요한 맥락이 슬라이드 밖에 있다»의 실물 근거다.
      ⚠️ **레인으로 만들지 마라** — 「이 두 장이 같은 맥락인가」는 파일 안에서 기계로 못 가른다.
      억지로 레인을 만들면 그게 장식이다. 가능한 것은 **관측 한 줄**뿐이다:
      «연속 N장 제목 동일» 분포를 **세어서 보여주기만**(판정 없이). 저자가 자기 덱의
      지붕 구조를 눈으로 보는 용도다.
