...
⚠️ **Count with `grep -o … | wc -l`, never `grep -c`.** The page is minified, so every anchor sits on
one line and `-c` (which counts LINES) reports **1** where there are 5. Measured 2026-09-16 while
re-verifying this very block — a leg that trusts `-c` silently caps the Batch count at one.
