...
(`sidecar_calibrate.sh` 는 detector 라 **항상 exit 0** 이고, 패널 편입 규칙도 불변이다).
⚠️ 다만 **`sidecar_panel_*.txt` 를 grep 하는 쪽은 갱신이 필요하다** — `UNTRUSTED-PIN` 만
세면 새 두 값을 놓친다.

사이드카 핀 판정이 **두 값**이라 «답이 아예 안 왔다» 와 «답은 왔는데 다른 모델이었다» 가
같은 `UNTRUSTED-PIN` 으로 접혔다. 그래서 `sidecar_panel_2026-09-18.txt` 가 agy 를 09-14
UNTRUSTED · 09-17 PIN-OK 로 적고 «어느 쪽이 참인지 미해결» 로 남겼다 — 09-14 쪽은 **2m50s
타임아웃**이었으니 그건 판정이 아니라 **측정 실패**였는데 판정으로 렌더됐다.

    PIN-OK          신원이 핀과 맞다
    UNTRUSTED-PIN   **답은 왔는데** 신원이 다르다 — 바꿔치기. 이 도구의 표적
    PIN-BLOCKED     한도/쿼터/인증 — **채널이 막힌 것**이지 모델 문제가 아니다
    PIN-UNMEASURED  답이 아예 안 왔다 — 측정 실패. 0 이 아니다

🟥 **패널 편입 규칙은 한 글자도 안 바꿨다** — `PIN-OK` 만 든다. 미측정이 패널에 끼면 그게
fail-open 이고 이 변경의 의미가 정반대로 뒤집힌다. 레인 2d 가 그것을 직접 단언한다.

구현 곁가지: `_run … | _answer` 가 런타임 rc 를 버려서 두 사건이 같은 빈 문자열이 됐다.
`out=$(cmd); rc=$?` 로 먼저 잡는다(`${PIPESTATUS[0]}` 은 bash 전용이라 안 쓴다).

레인 21(+4): 스텁 `silent-timeout`(출력 0 · exit 124) · `quota-exhausted`(429 · exit 1) 신설.
양성만이 아니라 **«틀린 값이 안 붙나»** 까지 잰다. 되돌림(4값 → 2값, 검출기 불변) 21/0 → **17/4**,
적색 넷이 전부 신설 다리이고 편입 불변 다리는 초록으로 남는다.

🟥 **잔여**: agy 09-14↔09-17 갈림은 아직 안 닫혔다 — 이 변경은 그걸 **닫을 수 있게** 만든 것이다.
`PIN-BLOCKED` 의 grep 어휘는 스텁으로만 쟀다(과탐 방향이 열려 있다). `ollama` 경로는 안 건드려
두 경로의 어휘가 갈린다(`PIN-OK` vs `PIN-OK(envelope)`) — 통일은 별건.


### [3.18.0] — 2026-09-23 — 리콜 트리거가 문자열 일치에 걸려 3중 1만 뜨던 자리

**minor — 행동을 바꾸는 교리 한 줄.** 소비자의 게이트 수용은 안 깬다(`BREAKING (gate):` 없음).

`CLAUDE.md` §Autonomous Initiative Layer 의 **CATALOG-first recall** 행이 한국어 3인칭
회상 어구만 나열하고 있었다. 실사용 문장 셋으로 재 보니 **3중 1만 발화**했다 —
「지난주에 뭐 했지」는 걸리는데 「이전에 한 작업 찾아줘」·「find past work on X」는 안 걸렸다.
같은 의도인데 어휘가 다르면 라우팅이 통째로 빠지고, 그 결과는 «세션 파일을 하나씩 훑는»
경로다(이 행이 존재하는 이유가 정확히 그걸 막는 것이다).

⇒ 일반형 어구 셋을 추가했다. 🟥 **어휘를 늘린 것이지 판별자를 바꾼 게 아니다** —
이 행의 한계(어휘가 안 겹치면 여전히 못 잡는다)는 그대로고, 그 사실을 여기 적어 둔다.

부수: 디스패치 원장에 2026-09-23 항목(팔 둘, 신원 확인분만 — tally 와의 차이는 귀속 불가로 남김).

### [3.17.0] — 2026-09-22 — 계기가 「안 본 축」을 초록으로 답하던 자리 넷

**minor — 게이트 레인 넷을 조였고, 그중 하나는 소비자의 게이트 수용을 깬다.**

> 🟥 **BREAKING (gate): `test_preprep_drift_anchor.sh` D2 가 이제 «확장자 무관» 으로 본다.**
> 종전에는 `find -name '*.py'` 라 정본 41파일 중 **15개(37%)** 를 아예 안 봤다 — 그 사각에
> `SKILL.md` · `presentation_checklist.md` · 판정 기준선 · **R3 레인의 known-pair 픽스처
> (`fixture_R3_{positive,negative}.pptx`)** 가 전부 들어 있었다. 실측으로 그 픽스처 둘이
> 실제로 갈려 있었고(배포본 `923b8f23…` ↔ 정본 `3107ff54…`), 그동안 앵커는 초록이었다.
> **처방**: 배포본을 정본에서 다시 뽑아라(`git archive <커밋> -- <스킬경로> | tar -x`).
> 정본→배포본 **한 방향**이라 배포본 «전용» 파일은 이 검사의 대상이 아니다.

## 🟥 한 뼈대 — 「성공한 명령」과 「내가 물은 것에 답한 명령」은 다르다

이 릴리스의 네 수리가 전부 같은 얼굴이다. 명령은 `rc=0` 을 냈는데 답한 질문이 달랐다.

| 자리 | 안 본 축 | 어떻게 드러났나 |
|---|---|---|
| `score_policy_lens.sh` | 빈 `--runs` 가 `rc=0` · `RISK: NOT HIGH` 를 **HIGH 로 뒤집음** · 여러 줄 `QUOTE:` 의 지어낸 이어짐이 `GROUNDED` · 필드 중복이 조용히 첫 줄 승 | 외부 계열(codex/gpt-5.5) 단독 발견, **자력 적발 0** |
| `new-code-anchor` | 그 채점기를 **아무 러너도 안 불렀다**(caller 0) — 「스위트 초록」이 그 파일에 대해 아무 말도 안 했다 | CI 가 `MENTION_ONLY` 로 적발 |
| `test_preprep_drift_anchor.sh` | 위 BREAKING | 배포본을 다시 뽑던 피어 세션이 실측 |
| `test_multibyte_bracket_lint_lanes.sh` | 브래킷 안 `\xNN` — **BSD grep 은 죽고(rc=2) GNU grep 은 딴 걸 읽는다.** 게다가 `\|\| echo 0` 이 그 rc=2 를 «0줄» 이라는 **측정값**으로 접었다 | 🟢 **CI 만 읽다가 로컬을 직접 돌려서** 나왔다 — CI 는 같은 커밋에서 초록이었고 그 초록은 «틀린 이유로» 난 것이었다 |

## 그 밖에

- **`AGENTS.md` §Mandatory Non-Claude Checklist 1-b · `docs/codex-compat.md` §Known limitations 6**
  — 비-Claude 진입점이 「이 체크아웃에서 훅이 안 배선됐을 수 있다」를 **한 번도 말한 적 없었다**
  (`grep -c`: CLAUDE.md 3 vs 둘 다 **0**). 🟥 그 실패는 무음이다 — 통과한 게이트와 안 돈 게이트는
  터미널 출력이 둘 다 없다.
- `score_policy_lens.sh` 와 그 레인은 **출하하지 않는다**(`ACCEPTED_ABSENT`) — 판정 대상이 236K
  짜리 실험 아카이브라 소비자 능력이 아니다. 소비자 트리에서는 **SKIP 으로 이름이 찍힌다.**
- 되돌림 프로브에 네 번째 칸을 더했다 — **적용 확인이 통과해도 무력한 변이일 수 있다.**
  판별자는 하나다: **되돌림에서 초록은 통과가 아니라 계기 오류다.**

### [3.15.0] — 2026-09-20 — 🟥 3.14.0 의 주장 하나를 철회하고, 그 주장을 참으로 만든다

**minor — 새 게이트 레인(플러그인 전수 순회) 하나. 그리고 이 릴리스의 절반은 정정이다.**

## 🟥 RETRACTED — 3.14.0 의 `BREAKING (gate)` ② 는 거짓이었다

3.14.0 이 이렇게 적고 npm 으로 나갔다:

> 🟥 **BREAKING (gate): `count_check.sh` 가 플러그인 전수를 본다** — 종전에는 이름 ENUM 을 돌아
> **4개 중 2개만** 검사했고, 안 보는 플러그인의 선언 드리프트는 COUNT-CHECK PASS 였다.

**앞 절반은 거짓이고 뒤 절반은 참이다.** #763 이 실제로 한 것은 `fh-commons` **단언을 조인 것**
(agents 수치 추가 · marketplace per-plugin 스코핑)이지 **전수 순회가 아니다.**
`fh-preprep` · `fh-qp` 는 그 뒤로도 **안 세고 있었다.**

🟥 **원인은 읽기다**: #763 커밋 본문의 «문제 서술»(*"count_check.sh 가 플러그인 이름을 ENUM 으로
돌아 4개 중 2개만 본다"*)을 «수리 서술»로 읽어 CHANGELOG 로 옮겼다. 확인 대신 인용한 것이고,
이 레포가 [[feedback_half_fix_propagation_boundary]] 로 이름 붙인 전파 경계다.

**어떻게 잡혔나**: «대상 목록을 손으로 적은 계기가 몇 개인가» 를 세려고 스캔을 돌렸는데 **0건**이
나왔다. known-positive 가 실재한다고 알고 있었으므로 0 을 안 믿고 계기를 교정했고, 교정된 계기가
`count_check.sh` 를 «4중 2, 전수 순회 없음»으로 지목했다 — **내 릴리스 문장과 정면으로 어긋났다.**
🟥 즉 이 정정은 성실함이 아니라 **두 신호가 어긋나서** 나왔다.

## 🟥 BREAKING (gate): 이제 진짜로 전수를 본다 — 그리고 «선언 없음»이 FAIL 이다

`plugins/*/` 를 **디스크에서 열거한다**(이름 손적기 0). 각 플러그인마다 `plugin.json` **과**
`marketplace.json` 양쪽의 선언 개수를 디스크와 대조한다.

**한 줄 처방** — 모든 플러그인의 `description` 에 `N skills` 를 적어라.
🟥 **선언이 없으면 PASS 가 아니라 FAIL 이다** — 대조할 것이 없는 상태는 통과가 아니라 **미측정**이다.
디스크에 agents 가 0 이면 agents 선언 부재는 정합으로 본다(적을 것이 없다).

**이 레인이 켜자마자 실물 둘을 잡았다**:

    FAIL sweep: fh-preprep skills — description 에 개수 선언이 없다 (디스크 1). 부재≠통과
    FAIL sweep: fh-preprep marketplace skills — 개수 선언이 없다 (디스크 1). 부재≠통과

둘 다 이번 릴리스에서 선언을 채워 닫았다(`fh-preprep` = **1 skill**).

🟥 **marketplace 까지 같은 순회에 태운 이유**: plugin.json 만 전수 보고 marketplace 를 4중 2로
두면 **이 절이 고치려는 바로 그 반쪽-픽스를 재현한다.** 첫 구현이 실제로 그 상태였고, 넓히자마자
위 두 번째 FAIL 이 나왔다.

**죽은 컨트롤 방어**: 열거가 2개 미만이면 «전부 통과»와 «아무것도 안 봤다»가 같은 침묵이므로
**계기 오류로 FAIL** 한다.

## 검증

- **fail-before** — 순회를 켠 첫 실행이 `fh-preprep` 미선언으로 **rc=1**. 나머지 셋은 통과 ⇒
  전부 빨개지는 레인이 아니다
- 선언을 채운 뒤 **COUNT-CHECK PASS**, 12 단언 전부 초록
- 계기 교정 — 손적기 스캐너 1판이 **0건**을 냈고 그것이 **죽은 계기**였다. known-positive 로
  교정한 2판이 7파일을 냈고 그중 유일한 커버리지 계기가 `count_check.sh` 였다

## 🟥 안 닫은 것

- **3.14.0 은 이미 나갔다.** 그 CHANGELOG 의 거짓 문장은 그 버전 tarball 안에 남는다 —
  이 정정은 **다음 버전 기록면**에서만 유효하다
- 손적기 스캔의 나머지 6파일은 **커버리지 계기가 아니라 범위가 정당한 것들**로 손검증했다
  (`frontier_digest_*` · `chamber_run` 등). 🟥 그 판정은 **내 눈**이고 기계가 아니다
- 순회는 `description` 의 **첫** `N skills` 패턴만 읽는다. 한 설명 안에 여러 수치가 있으면
  첫 것을 취한다 — 과탐/미탐 방향 미측정

### [3.14.0] — 2026-09-19 — 계기가 «못 읽은 것»을 «깨끗함»으로 내던 자리 셋

**minor — 새 자산 넷(바이너리 추출기 · PR 없는 델타 스캐너 · 체크리스트 unblocked 훅 · 다중 턴 시뮬 경로)
과 게이트 수용을 깨는 변경 셋.**

🟥 **BREAKING (gate): 바이너리·비텍스트 파일이 더 이상 무음 통과하지 않는다** — `psa_scan_file` 이
NUL 바이트 + `file` mime 2축으로 판별해 스캔 불가 파일에 `UNSCANNABLE`(exit 3)을 내고, **exit 3 이
히트를 지배한다**. 종전에는 `grep -a` 가 zip 안을 못 읽고 **rc=0 CLEAN** 을 냈다.
한 줄 처방 — pptx/docx/pdf 가 출하 집합에 있으면 `scripts/psa_extract_text.py` 가 필요하다
(OOXML 은 zip 의 `.xml`/`.rels` 전 파트, PDF 는 PyMuPDF). 추출기가 없거나 0자를 내면 **exit 3** 이다.
목록은 **스캔가능 쪽**에 둔다 — 모르는 형식은 UNSCANNABLE 로 떨어지는 fail-closed 방향이다.

🟥 **BREAKING (gate): `count_check.sh` 가 플러그인 전수를 본다** — 종전에는 이름 ENUM 을 돌아
**4개 중 2개만** 검사했고, 안 보는 플러그인의 선언 드리프트는 COUNT-CHECK PASS 였다.
한 줄 처방 — 각 플러그인의 README/매니페스트 선언 수를 디스크 실물과 맞춰라. 안 맞으면 이제 FAIL 이다.

🟥 **BREAKING (gate): `session_checklist.py` 에 `unblocked` 와 rc=4 가 생겼다** — 형식은 멀쩡한데
**블로커가 DONE 으로 닫힌 뒤 아무도 다시 안 읽은 행**을 찾아 낸다(종전 check 는 같은 파일을
rows=14 ok=14 violations=0 rc=0 으로 통과시켰다). rc=4 는 **DEAD CONTROL** — 의존을 하나도 못 읽고
«0 건» 을 내는 계기는 아무것도 측정하지 않았다는 뜻이고, «다 해소됨» 과 구분된다.
한 줄 처방 — 체크리스트 행이 블로커를 명명하면 그 블로커가 닫힐 때 해당 행을 다시 읽어라.
🟥 훅은 현재 `rc=1` 만 보고하고 **`rc=4` 는 조용하다** — 안 닫힌 자리로 명시한다.

- **첫 실사용이 실물 유출을 잡았다 (#762)** — 수리 직후 진짜 발행 게이트를 출하 집합(npm pack 482파일)에
  돌리자 출하되는 pptx 픽스처 **둘 다** 서체명에 회사 식별자를 품고 있었고, 옛 `grep -a` 가 여태
  통과시켰다. 서체명 중립 치환·재압축 후 preprep 레인 **163 passed · 0 failed 로 전후 동일**,
  게이트 rc=1 → **rc=0**.
- **PR 없는 델타 스캐너 (`scripts/prless_delta_scan.sh`, #763)** — 마감 ①-b 는 `--state open` 이라
  PR 이 없는 브랜치를 **구조적으로** 못 본다. 판별자는 «브랜치 델타 합본의 patch-id»(squash 머지는
  정확히 그 한 장을 main 에 심으므로 일치한다). 실물 40 브랜치 — naive 33 · `git cherry` 17 · 이 계기 10.
  🟥 **advisory 다(rc=1)** — 삭제는 Destructive-Op 관할이고 이 계기는 «세는 것» 만 한다.
- **다중 턴(티키타카) 시뮬 경로 (`sim_isolated_run.sh --turns`, #759)** — 단발 `-p` 는 «발화하는가» 를
  잰다. 턴을 넘어야만 존재하는 축이 따로 있다: 협상이 **수렴**하는가, 그 절충안이 다음 턴에 **묶이는가**.
  known-pair 를 실물 `claude` 로 갈랐다(변수는 `--resume` 하나) — ARM 은 턴2 가 답하고 session_id 하나,
  CTRL(`--no-resume`)은 «말씀하신 적이 없어요» 에 session_id 둘. 🟥 **단발 경로는 바이트 무변경**이고
  L28j 가 argv 로그로 그것을 단언한다. 채점기 = `scripts/tikitaka_score.py`(+ known-pair 픽스처 11종).
- **liveness 증거 에코를 집계 판정 토큰에서 분리 (#764)** — selfcheck 는 스위트 출력의 `❌` 를 grep 해
  실패를 판정하고, 되돌림 프로브는 «가드를 꺼서 레인이 빨개졌다» 를 증거로 그 빨간 줄을 되찍는다.
  두 기계가 각각 옳고 **이음매는 아무도 안 본다.** 지금 뚫리진 않지만(프로브 rc=0) 사람은 뚫렸다 —
  같은 CI 로그를 두 번 오귀속했다. 증거 에코만 `[expected-red]` 로 재토큰화했고, **집계기에 예외
  목록을 만들지 않았다**(그러면 다음 프로브에서 또 샌다).
- **Sonnet-floor 교리 2절 추가 (#764)** — «최소사양» 정식화: 플로어는 우리가 실제로 돌리는 **가장 작은
  런타임**이고, 거기서 한계는 티어가 아니라 **컨텍스트**다.
- **논문 기록면 정리 (#760·#761·#765)** — 발행본 v1.0.1 은 발행본대로 두고 편집본을 v1.0.2 로 분리,
  arXiv 에 실제 제출된 판을 v1.0.3 으로 기록. 서막에서 «결과» 가 아닌 것을 빼고 모순 넷을 고쳤다.

**안 닫은 것**: 체크리스트 훅이 `rc=4` 를 조용히 삼킨다 · 티키타카 F1·F5(회차 «간» 비교) 집계기 0줄 ·
`prless_delta_scan` 의 stacked 브랜치 과탐지(advisory 방향이라 의도) · 마감 체인에 미배선 ·
`count_check` 일반화의 나머지 스크립트 약 40개 미확인 · psa 추출기는 OOXML/PDF 만 덮는다.

### [3.13.0] — 2026-09-19 — 규칙은 있었고 배선이 없었다, 두 자리

**minor — 새 자산 하나(발신 전 3프로브 훅)와 발행 확인 계기 하나가 늘었다.**
🟢 **게이트 수용을 깨는 변경은 없다** (`BREAKING (gate):` 줄 불요) — `outbound_pr_gate.sh` 는 파일로
출하되지만 **어떤 settings 스니펫에도 배선되지 않는다**(기계 확인: 컨트롤 `outbound_query_hook` 은
...
- **발신 전 3프로브 훅 (`scripts/outbound_pr_gate.sh`, #754)** — PreToolUse(Bash), 비소유 레포에 PR 을
  ...
  규칙(§Skeleton-Not-Muscle · §7 standpoint enum)은 이미 있었고 **배선이 0** 이었다.
  레인 25(경로/인용 형태 · 변수간접 **잔여 핀** · enum 클래스닫힘 · 되돌림 프로브).
- **발행 «확인» 을 발행 «판정» 에서 분리 (`scripts/publish_verify_poll.sh`, #754)** — verify 단계는
  발행을 판정하지 않는다. 앞 단계 `npm publish` 가 이미 rc=0 이므로 여기서 보는 것은 읽기 복제본 전파뿐이다.
  종전 60초 인라인 루프가 **성공한 v3.12.0 발행을 빨간 잡으로** 만들었고(판별 = 레지스트리 멱등성),
  그 빨강이 v3.2.0·v3.4.0 에서 손 발행을 훈련시켜 OIDC 경로를 깨뜨렸다. 예산 5분 + 판정은 `outcome=`,
  종료코드는 **계기 오류에만**. 레인 16.
- **cross-family(codex)가 S급 2 + A급 4 를 냈고 넷을 고쳤다** — `|| true` 가 계기 오류를 삼켜 조회기
  부재를 `not_yet_visible` 로 내보내던 것 · 출력 채널 부재 시 판정이 사라지던 것 · `${{ }}` 셸 주입 ·
  경로/인용 형태 트리거 미탐. ⚠️ **A3 주장 7건 중 5건은 실측으로 반증**했다(종전 정규식으로도 잡힌다) —
  받은 것을 그대로 옮기지 않았다.
- 🟥 **selfcheck 와 CI 가 저자의 배선 구멍 셋을 잡았다** — 레인을 페어링 «선언만» 하고 실행 블록 미첨부 ·
  출하 문서가 이름을 대는데 `files[]` 부재 · 스텁을 shim 으로만 간접 호출(MENTION_ONLY/zero-caller).
  마지막 것을 선언(EXEMPT)으로 안 빼고 **직접 실행 레인**으로 닫자 **스텁 자기 계약이 한 번도 검증된 적
  없었다**는 것이 드러났다.

**안 닫은 것**: 변수 간접(`$GH_CMD`)·alias·gh 래퍼는 정규식으로 못 닫는다(레인이 **고정만** 한다) ·
`gh pr edit --body` 는 NAMED GAP · `echo gh pr create` 과차단 1건(막는 쪽이라 유지) ·
소비자 install 에서 publish-verify 레인은 **SKIP**(SKIP 은 PASS 가 아니다).

### [3.12.0] — 2026-09-18 — 논문이 계기를 낳고, 계기가 저자를 잡은 밤

**minor — 새 계기(논문 무결성 3종 · 철회 숫자 전파 스캐너 · 세션 요청 체크리스트)가 늘었고, `fh-codex-doctor` 의
`--json` 계약이 바뀐다(`inputErrors` 필드 · 종료코드 붕괴 3건 수리).** 게이트 수용을 깨는 변경은 **없다**
(`BREAKING (gate):` 줄 불요 — 새 계기는 전부 advisory 이고 커밋 게이트에 안 건다).

- **`fh-codex-doctor` 과차단 수리 (#745)** — 출하 바이너리가 **읽지도 않는** `package.json` 으로 문을 잠가
  모든 비-npm 소비자를 거절했다(rc=11). 같은 결함족 둘 더: 빈 `plugins/` 가 «OK · 0 scanned · rc=0»(역방향) ·
  `collectAgents()` 하드코딩 네임스페이스. cross-family 가 종료코드 붕괴 3건 추가 적발. `--json` 에
  `inputErrors` — 옛 rc=11 의존 호출부 0건 확인.
- **논문 무결성 계기 3종 (#749)** — `citation_key_check.py`(인용키 used↔defined + 근사키 + MALFORMED) ·
  `numeric_token_diff.py`(재작성 전후 숫자 토큰 DROPPED/INVENTED — 부호·지수·전각·범위) · `section_ref_check.py`
  (§/Section/섹션/N절/부록/K.n → 헤딩 해석, 범위 내부 포함). 레인 73. 🟥 cross-family 열여덟 라운드가 무음 결함
  40 을 하나씩 잡았다 — 같은 계열 known-pair 는 매 라운드 초록이었다. 실논문에 실제로 돌려 고아 인용 1 을 편집.
- **철회된 숫자의 전파 스캐너 `claim_propagation_scan.py` (#748)** — 정본이 철회한 수치가 다른 파일에 살아남는
  것을 잡는다(식별자 전파는 `halffix_propagation_scan.sh` 가, 주장 전파는 이것이). 레인 28.
- **세션 요청 체크리스트 `session_checklist.py` (#750)** — 운영자 발화를 두 채널(raw 전사본 + 압축 요약)에서 전부
  뽑아 «달성 상태 · 사유 · 이월 제안» 결과서의 형식을 검사한다. 공정 말미의 «영혼 다지기». 첫 실사용에서 저자의
  손표 한 행을 잡았다. 마감 체인 미배선(advisory).
- **정본·상주층 정정** — 5팔 표(2.7~13.6 %) 철회 → 4팔 스펙트럼(0.0~1.1 %)로 교체(#743) · «Axis 1 은 안 돈다» 낡은 CI
  서술 여섯 자리 정정(#746) · 간판 둘째 동사 «상주» → «플로어에서 발화한다(측정)»(#747) · 선행조사 4·5라운드 +
  formal model(#744) · `predelete_check.sh` 가 유령 원격 ref 236 을 회수 대상으로 보고하던 것 수리(#742).

### [3.11.0] — 2026-09-17 — 세 번째 레지스터, 그리고 「목록을 손으로 다시 적던」 자리들

**minor — 새 자산(낭독 레지스터)이 늘었고 행동이 바뀐다.** 게이트 수용을 깨는 변경은 **없다**
(`BREAKING (gate):` 줄 불요 — sync 변경은 «덜 실어보내는» 방향이지 «새로 막는» 방향이 아니다).

- **ko-tech-writer 낭독 레지스터(Step 5-s)** — 읽는 산출물 둘에 더해 **소리 내어 읽히는** 산출물을
  다룬다. 낭독 대본은 이제 Step 5 를 N/A 로 비우지 않는다(렌더 매체가 소리다). Step 2 클래스표를
  `S1~S9` + check-class 로 재작성하고 `Step 2-a`(새 계기 짓지 마라) 신설. `fh-preprep` 과의
  역할경계 명시 — preprep 이 `L9` 를 빼며 「번역투·레지스터·수치게이트·지각 QA 는 ko-tech-writer
  로」라고 **먼저 그어 놓은** 자리다. 🟥 `P1`~`P3` 는 아직 레인이 없고 `rg` 부재 시 degrade 경로를
  명시했다(바꾸기 전에 known-pair 로 확인하라 — 정규식 방언이 다르면 조용히 0건을 낸다).
- **live_eval `G-TRIG-01` 제외** — 2026-07-17 에 삭제된 CLAUDE.md 라우팅 행을 **50일 동안** 재고
  있었다. 쌍둥이 `G-TRIG-03` 은 이미 같은 사유로 제외돼 있었는데 한쪽만 옮겨졌다(반쪽 픽스).
  Overall 이 12 프로브 기준으로 계산된다. 🟥 「row-diet 위임이 플로어 티어에서 발화하는가」는
  여전히 **UNMEASURED** — 경고를 닫았지 질문을 닫지 않았다.
- **sync 제외 목록을 배열로 결박** — `SYNC_EXCLUDES` 가 이미 `.git/` 를 갖고 rsync 경로는 그것을
  쓰는데 **tar 폴백만 목록을 손으로 다시 적고** 있었다. 표기 차이(rsync `logs/` ↔ tar `logs`)가
  그 이유였고 `${e%/}` 변환이 그 이유를 없앤다. 복귀 경로에도 `.git` 제외. 레인 4팔(A·**C**·E·R).
- `dataviz` 명제 정정 — 「이 레포에 그런 스킬은 없다」는 **거짓**이었다. 런타임 번들 스킬로 실재하고,
  「레포 자산이 아니다」를 「없다」로 접으면 실재하는 스킬로 가는 포인터를 잃는다.

### [3.10.0] — 2026-09-16 — 「조용히 0을 내던」 두 자리와, 무시된다고 믿었던 서브트리

**BREAKING (gate): pre-commit 에 gitlink 차단 레인이 생겼다 — `tracks/` 밑에 자기 `.git` 을
가진 디렉터리가 있으면 커밋이 막힌다.** `.gitignore` 의 `tracks/**` 는 **파일**만 무시하고,
중첩 레포는 mode `160000` 엔트리로 인덱스에 올라간다. 처방: 그 서브트리가 기록 증거라면
`TRACKS_GITLINK_OK=1` 로 통과시키고(사실이 출력에 남는다), 아니면 중첩 `.git` 을 치워라.
🟥 범위는 **일부러 비대칭**이다 — `tracks/` 밖의 gitlink 은 **표면화만** 한다(정당한 서브모듈을
과차단하면 `--no-verify` 를 훈련시키고, 그건 같은 훅의 Destructive-Op 게이트까지 끈다).

이번 판의 공통 축: **실패가 «0» 으로 위장하던 자리.**

- **frontier-digest The Batch 다리가 죽어 있었다.** 지시된 추출식(`"slug":"issue-N"`)이 살아있는
  페이지에서 0매치인데 HTTP 200 이라, 그 다리는 실패가 아니라 「Batch 0 items」를 냈다 —
  매일 돌면서. 현행 형태(`aria-label` + href)로 교체하고, **0매치의 해석을 규칙으로 박았다**:
  200 에 0매치는 계기 불일치이지 0건이 아니므로 `UNMEASURED` 로 보고하고 집계에서 뺀다.
  ⚠️ `grep -c` 금지 — 페이지가 미니파이라 5매치가 1줄이고 `-c` 는 1을 낸다.
- **gitlink 은 커밋 출력 어디에도 안 나타난다.** 클론이 그 SHA 를 못 가져와 트리는 멀쩡해
  보이는데 내용이 없다. 같은 뿌리의 sync 표면(3.9.x)은 시끄러웠고 이쪽은 조용하다.
- 에어 노드 디스패치 원장 2건 회수(3주 지연 — PR 없는 브랜치를 마감 스윕이 구조적으로 못 본다).

### [3.9.0] — 2026-09-14 — 「measured 라고 선언해 놓고 안 재던」 세 자리를 실행으로 바꾼다

**BREAKING (gate): `selfcheck.sh` 에 temper_check 레인 블록이 생겼다 — 그 레인이 실패하면
소비자의 `selfcheck` 가 빨개진다.** 처방: `bash scripts/test_temper_check_lanes.sh` 를 직접 돌려
실패 레인을 보고, `templates/temper_check.sh` 가 설치본에 있는지 확인해라(부재는 SKIP 이 아니라
FAIL 로 접는다 — files[] 에 있는 자산의 부재는 «정당한 미출하» 가 아니라 삭제다).
검증 환경: bash **3.2.57 · 5.3.15** 둘 다 49/0 · 얕은 클론 45/0/2 SKIP.

이번 판의 공통 축은 하나다 — **선언이 계기를 앞질러 있던 자리**.

#### `temper_check.sh` — `measured` 선언이 `bash -n` 에 얹혀 있었다 (#724)

게이트체인 7경로 중 **유일하게 실행 레인이 0** 이었다. 참조 6곳이 전부 비실행이고,
🟥 **그중 하나가 `selfcheck.sh` 자신의 `bash -n` 목록**이었다 — selfcheck 가 「구문 검사」를
「검증」으로 세고 있었다. 그 사이 `steel-quench` SKILL.md 의 T-1 은 그것을 `measured` 로 선언한다.

새 레인 **49/0/0**. P1/N1 만으로는 광고된 분기를 **한 번도 안 지나간다**:
- **P2** 증가분이 전부 펜스 안 → 「fence interiors excluded」
- **P3** 줄어든 파일 → 「simplification un-punished」(`abs()`/클램프가 끼면 조용히 거짓이 된다)
- **E1 ×4 + E1-control** — 「못 쟀다」가 「Δ=0」으로 안 읽히게, 그리고 **판별자가 죽어서 나온
  «표 없음»이 아님**을 컨트롤로 증명

되돌림 3단(적용확인→실행→복원 해시 동일)에서 **P2 만** 적색이고 값이 예측과 정확히 일치
(Δsections +23 · steps +2 · tables +6 = 펜스 내부 위장 줄 수).

🟥 **발견한 결함을 기대값으로 고정하지 않았다** — `fences` 세는 법이 두 군데서 다르다
(`grep -c '```'` 포함 vs 토글 `^[[:space:]]*```' 줄시작). 덮는 레인을 **일부러 안 만들었다**.
🟥 **남은 구멍**: 「T-1 을 누가 부르는가」는 여전히 산문이다 — SKILL.md 의 명령줄은 지시문이지
호출부가 아니다.

#### live_eval — `G-TRIG-07` 은 10런 전패였는데 원인이 자산이 아니라 계기였다 (#723)

행동은 3/3 옳고(디스패치 거부 + 되묻기) **리터럴만 1/3**. 프로브가 「스킬 이름을 입에 담았나」로
재고 있었다. 사전등록 후 6팔 18회를 돌려 본문을 저장하고 같은 본문에 두 패턴을 채점했다:
구 패턴 **0/3**, 새 패턴 **2/3**, 컨트롤 5종 전부 0/3.

🟥 **초록으로 덮지 않았다** — `advisory_re` 채널이 판정에 **관여하지 않고** 「제안 형식을 안
따랐다」를 매 런 숫자로 남긴다(현재 `advisory[deep-clarify] 0/3`).

곁들여: **증거 보존**(런 본문이 `mktemp` 로 소멸해 09-12·09-13 원인이 영구 미귀속이었다) ·
**RUN-BLACKOUT**(계기 사망을 `FAIL` 이 아니라 `UNCALIBRATED` 로) · `G-GREET-02/05` 편입
(**03 은 «못 짰다»고 적었다** — 분기 렌더가 비결정적이라 정규식을 박으면 옳은 답을 빨갛게 찍는다) ·
warning 2건 해소. 레인 **41 → 61**.

🟥 **소비자 대면 실결함 하나 수리**: `test_probe_live_eval_lanes.sh` 는 출하물인데
`.claude/regression/probes.md` 는 안 실린다 — 깨끗한 설치에서 레인이 traceback 으로 죽고
**`RESULT: REGRESSION`**(= 픽스처 부재를 판정으로 렌더)을 찍었다. `exit 10` HARNESS-ERROR 가드.
**probes.md 출하 여부는 안 건드렸다.**

#### preprep — cross-family R5, 그리고 R4 의 「닫았다」 기록이 거짓이었다 (#722)

R4 의 기함 수리와 R3 의 rels 수리를 **통째로 되돌려도 self-test 30/30 초록**이었다. R4 는
「1장 픽스처는 폴백과 같은 답을 낸다」를 정확히 진단하고 2장으로 바꿨는데 — **그 2장도
발표순 = 파일명순이라 안 꼬여 있었다.** 레인 **30 → 42**, 신규 S4·A2·B1, **NOT-CONVERGED**.

계열 탈상관은 약하다(세 팔 전부 OpenAI — Gemini 쿼터 0%). 출처 축만 갈랐다
(`evidence=INDEPENDENT`, 파일 교집합 0). 실물 덱 804회 결과는 수리 전후 **동일**.

#### governance — supersede 된 버전 DOI 를 concept DOI 로 (#721)

`governance_engineering_definition.md` 의 Zenodo 링크가 v1.2.1 에 도착하고 있었다.
일괄 치환하지 않았다 — 옛 DOI 가 **옳게 쓰인 자리 2곳은 그대로 뒀다**(수리 후 3→2 가 범위 컨트롤).


### [3.8.0] — 2026-09-14 — 플로어 없는 채널의 4축 마커 + 트랙 해석 규약을 좁힌다

**⚠️ 소비자 동작 변화 하나 — `fh_resolve_track_root` 의 별칭 해석 규약이 좁아졌다.**
같은 이름의 실물 레포와 `-dev` 포크가 **동시에** 있는 노드에서, 종전에는 `AMBIGUOUS`(rc=2)로
떨어지며 아무것도 안 골랐다. 이제 **정확 일치가 있으면 그것이 이긴다**(rc=0). 정확 일치가
**없을 때는 종전 그대로 AMBIGUOUS** 다 — 즉 막히던 것이 풀리는 방향이고 새로 막는 것은 없다.
(하류 제보 pmh-dev #80. 제안은 「first match wins」였고 **「exact match wins」로 좁혀서** 받았다:
 넓은 쪽은 정확 일치가 없을 때 후보 하나를 **조용히** 고르는데, 그게 이 가드의 존재 이유다.)

**신규 — `scripts/remote_marker_gate.sh`**
FH 자산 변경의 4축 마커는 `tracks/**` 에 살고 그건 gitignored 다. 저작 노드가 이 레포의 작업
트리면 맞는 설계인데, **원격 자율 노드**(체크아웃이 휘발하는 클라우드 세션)는 마커가 같이
사라진다. 그리고 CI 는 gitignored 파일을 구조적으로 못 본다 ⇒ **마커 없는 FH 자산 커밋이
초록으로 들어온다.** 실측: `claude/` 접두사 머지 PR 전수 **2/2** 가 그랬다.
⇒ 그 채널에 한해 «축 3줄(`axes-run`·`crossfamily`·`standpoint`)이 커밋 기록에 실려 왔나» 를
  `validate` 에서 본다. **보통 브랜치는 SKIP** — pre-commit 이 이미 관할하므로 이중 요구는
  override 를 훈련시킨다.
🟥 **형식만 본다.** 값의 진위도 enum 멤버십도 안 본다(그 정본은 저작 노드의 훅이다).
  옮긴 마커와 지어낸 마커는 바이트가 같다 — 닫은 것은 «조용한 부재» 하나다.

**문서**
- `asset-placement-gate` — 인용한 논문(arXiv:2605.11418)의 **네 번째 수치**를 채웠다:
  Governance 단계에서 semantic evasion 이 **차단 판정을 36.5%–100% 회피**. 입회 게이트는 정확히
  차단-판정 표면이라 이 게이트 자신의 사각을 가리킨다. 🟥 다른 레지스트리·다른 위협모형이고
  상단 100% 는 «어떤 구성은 아무것도 안 막았다» 라는 뜻 — 양방향으로 좁혀서 적었다.
- `self_evolution_routine.md` §5b 신설 — 자율 패스가 마커 3줄을 **커밋 메시지에** 싣도록.
- `CLAUDE.md` §4축 기계층 — 「CI 가 마커를 못 본다」 뒤에 그 사각이 실제로 뚫린 실측을 붙였다.

**레인**: `test_remote_marker_gate_lanes.sh` 15개(되돌림 프로브 포함) · `test_track_resolve_lanes.sh`
24→28 · `test_adapter_lanes.sh` 37→38 · `cluster_capability_scan --self-test` 20→21.
🟥 그 과정에서 **기존 모호 컨트롤 5건(파일 3개)이 대상과 같이 죽을 뻔했다** — 기대값이 아니라
**픽스처의 과녁**을 «정확 일치가 없는» 케이스로 옮겨서 살렸다.

### [3.7.0] — 2026-09-13 — 리뷰 페르소나 캐스트 출하 (`persona-cast` + 렌즈 6)

필드 하네스가 **컨테이너를 몰라도** 쓸 수 있는 리뷰 좌석 묶음. 화면·스펙·PR·페이지를 여섯 좌석의
눈으로 **격리 디스패치**하고, 고유 발견 0인 좌석은 **장식으로 표기**한다(채워넣지 않는다).

| 좌석 | 보는 것 |
|---|---|
| `designer-lens` | 시각 위계 · 상태 완비(빈/로딩/오류/성공/부분실패) · 일관성 · 밀도 |
| `ux-writer-lens` | 행동 문구 · 오류 문구(무엇이·왜·어떻게) · 용어 표류 · 말투 |
| `fe-dev-lens` | 상태 전이(뒤로·새로고침·더블클릭) · 입력 경계 · 반응형 · 선택자 안정성 |
| `be-dev-lens` | 멱등성 · 권한 경계 · 데이터 정합 · 실패 계약(4xx/5xx/타임아웃) |
| `qa-lens` | 케이스 트리 도달성 · 사전조건 · 판정 기준 · 회귀 위험 · 재현성 |
| `a11y-lens` | 키보드 완주 · 이름 · 대비 · 색 이외 신호 · 동적 변화 알림 |

🟥 **핵심은 좌석 목록이 아니라 «티어 상한»** 이다 — 좌석마다 *What you may NOT assert* 가 박혀
있어서 좌석들이 같은 말을 하지 않고 **서로에게 넘긴다**. 그 칸 밖의 주장은 finding 이 아니라
*Open questions* 로 간다.

- `persona-cast` 는 **탐색 모드**(`--mode explore`)를 갖는다 — 좌석은 «다음에 건드릴 곳 · 무엇으로
  판정할지 · 정지 조건» 만 내고 **조작은 호출자가 한다**. 🟥 이 분리를 지켜야 「눌렀다」고 쓰면서
  아무것도 실행하지 않는 형태를 피한다.
- 트리거 프로브(라우팅 스킬 의무) 실측: 블라인드 플로어 티어 · reps=3 —
  **양성 3/3 · 음성 컨트롤 0/3**.

⚠️ **탐색 모드는 살아 있는 앱에서 미검증**이다. 리뷰 모드(고정 아티팩트)만 실측했다.

### [3.6.0] — 2026-09-13 — 사용 원장의 동의를 «첫 실행» 에서 받는다 (3상태)

`preprep` 과 필드 하네스 진입점이 **쓰이는 순간** 한 번 묻는다. 공지는 읽는 사람과 쓰는 사람이
다르기 때문이다(운영자 결정).

| `FH_USAGE_LEDGER` | 뜻 | 행동 |
|---|---|---|
| 미설정 | 아직 안 정함 | 매 실행 **stderr** 3줄 안내 |
| `<경로>` | 동의 | 조용 + 기록 |
| `off` | **명시적 거절** | 조용 + 기록 없음 |

🟥 **`BREAKING (gate)` 는 없다** — 게이트 수용이 깨지지 않는다. 켜든 끄든 도구 동작과
**종료코드가 완전히 같고**, 그 동일성을 레인이 직접 잰다(FH ⑫-6 · qasp ⑥-5).
그래서 안내 문구도 「안 켜면 제대로 안 돈다」라고 **쓰지 않았다** — 그건 거짓이고, 거짓으로 받은
동의는 그 동의로 얻은 숫자의 값도 같이 떨어뜨린다. 쓰는 이유는 참인 쪽이다:
**「당신이 어디서 막히는지 우리가 못 본다」**.

- 안내는 **stderr 전용**이다. `stdout` 은 판정 출력이고 기계가 읽는다 — 한 줄도 안 끼운다.
- 나가는 문이 거짓말이 아니라 `off` 다. **거절도 기록**이므로 정한 사람에게는 다시 안 묻는다.
- `$HOME` 앵커를 문구에 박았다(`export FH_USAGE_LEDGER="$HOME/.fh-usage"`) — 레포가 어디에
  있든 안내 문구가 모두에게 **글자 그대로 같다**.

⚠️ 명명된 잔여: 안내가 실제로 전환을 만드는지 **미측정**이고, **원장 자신이 그걸 못 잰다**
(안 켠 사람은 기록이 없다 — 구조적 사각).
