# fh-commons — 경합 출산 스킬 서식지

**fh-meta와 다른 층위의 플러그인.**

fh-meta = 하네스 자체를 운영·진단·고도화하는 메타 엔지니어링 스킬들  
fh-commons = 경합 레이어에서 탄생한, 어느 프로젝트에나 이식 가능한 공용 유틸리티 스킬들
...
| 판정 | 경로 |
|---|---|
| 하네스 엔지니어링 성격 | `fh-meta` |
| 프로젝트 공용 · 도메인 무관 | **`fh-commons`** (이 플러그인) |
...
| `deliberation` | Innovator → Devil-Advocate → Mediator 3-layer 다관점 합성. 이진 승패 없이 조건부 판정 생성 | fh-meta 이관 (2026-05-23 — 도메인 무관 범용 의사결정 구조) |
