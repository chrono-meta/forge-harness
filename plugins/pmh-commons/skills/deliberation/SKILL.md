...
> **fallback (deep-insight 미설치 시)**: `pmh-commons:quench-challenger` (Devil DNA 포함) 또는 CC 인라인 Devil-Advocate 역할 수행. 출력 형식 동일. Step 1과 마찬가지로 인스턴스 격리 불필요.
...
| 상황 | 연계 스킬 |
|---|---|
| 설계 결정 충돌 시 자동 발동 (Wave next-D) | `pmh-meta:agent-composer` Step 4-b |
| 완성된 자산 품질·정합성 검증 | `pmh-meta:sim-conductor` |
| deliberation 이후 후보 스킬 검증 | `pmh-meta:harness-doctor` |
| 결정 이후 구현 수렴 루프 | `pmh-commons:convergence-loop` |
