...
레인:
  ...
  L14 font      — 서체가 그 덱의 **배포 템플릿**이 정한 집합을 벗어났나 (lane_font.py · 2026-09-11).
...
def main():
    ...
    try:
        ...
        notes.append('L13 slide-refs : 계기 미실행 — NOT_WIRED (%s: %s) (0 아님)'
                     % (type(_e).__name__, _e))
    # L14 font — 서체가 «그 덱의 템플릿이 정한 것»을 벗어났나(2026-09-11 신설).
    ...
    try:
        import lane_font
        _f14, _n14 = lane_font.scan(cfg, root)
        findings += _f14; notes += _n14
    except Exception as _e:
        notes.append('L14 font : 계기 미실행 — NOT_WIRED (%s: %s) (0 아님)' % (type(_e).__name__, _e))
